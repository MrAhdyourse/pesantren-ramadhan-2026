    ...
    >
      ...
      <div className="relative bg-gradient-to-r from-emerald-600 via-teal-500 to-emerald-600 rounded-t-3xl p-8 text-white text-center shadow-2xl overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-20">
          <div
            className="absolute inset-0 animate-gradient-shift bg-gradient-to-r from-emerald-400/0 via-amber-400/50 to-emerald-400/0"
            style={{ backgroundSize: '200% 100%', willChange: 'backgroundPosition' }}
          />
        </div>
        ...
        <div className="relative z-10">
          <div
            className={`text-sm opacity-80 mb-2 ${mounted ? 'animate-slide-up' : 'opacity-0'}`}
            style={{ animationDelay: '0.1s' }}
          >
            {day.hijriDate}
          </div>
          <h2
            className={`text-4xl font-bold mb-2 ${mounted ? 'animate-bounce-in' : 'opacity-0'}`}
            style={{ animationDelay: '0.2s' }}
          >
            Day {day.dayNumber}
          </h2>
          <div
            className={`text-lg opacity-90 ${mounted ? 'animate-slide-up' : 'opacity-0'}`}
